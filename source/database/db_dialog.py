...
class DBDialog(dialog.DialogBase):
    ...
    def __init__(self, ctx, cast, logger, **props):
        ...
        # Use DatabaseConfigManager for config handling
    ...
    def _load_config(self):
        ...
        self.logger.info(f"Loading configuration from {self.db_config_manager.config_path}")
        saved_db = None
        try:
            # Load configuration using DatabaseConfigManager
            config_params = self.db_config_manager.get_connection_params()
            
            if config_params:
                self.host.Text = config_params.get('host', self.DEFAULT_HOST)
                self.port.Text = str(config_params.get('port', self.DEFAULT_PORT))
                self.user.Text = config_params.get('user', self.DEFAULT_USER)
                self.password.Text = config_params.get('password', self.DEFAULT_PASSWORD)
                saved_db = config_params.get('database')
    ...
    def _load_databases(self, *args):
        ...
        try:
            ...
            self.logger.error(f"Failed to import database connection module: {str(e)}")
            MsgBox("Database connection unavailable. Please check your LibrePy installation.")
            return
        
        # Pre-flight connectivity check
        import socket
        host = self.host.Text or self.DEFAULT_HOST
        try:
        ...
        self.logger.debug(f"Pre-flight connectivity check to {host}:{port}")
        try:
            ...
            if result != 0:
                self.logger.warning(f"PostgreSQL service not reachable at {host}:{port}")
                MsgBox(f"PostgreSQL service not reachable at {host}:{port}.\nPlease verify the server is running and accessible.")
                return
        except Exception as e:
            self.logger.warning(f"Pre-flight connectivity check failed: {str(e)}")
            MsgBox(f"Cannot reach PostgreSQL server at {host}:{port}.\nPlease check your network connection and server settings.")
            return
            
        self.logger.info(f"Loading database list from {host}:{port}")
        try:
            # Clear existing items
            self.database.removeItems(0, self.database.getItemCount())
            
            # Get connection parameters (port already validated in pre-flight check)
            conn_params = {
                'host': host,
                'port': port,  # Already validated as integer
                'user': self.user.Text,
    ...
    def _save(self, *args):
        ...
        try:
            ...
            # Save configuration using DatabaseConfigManager
            success = self.db_config_manager.save_connection_params(config)
            if not success:
                self.logger.error("Failed to save database configuration")
                MsgBox("Failed to save database configuration. Please check the application logs for details.")
                return

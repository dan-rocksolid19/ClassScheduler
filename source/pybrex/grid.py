...
class GridBase(object):
    ...
    def get_row_index_from_event(self, ev, require_double_click=False):
        """Return the row index associated with a mouse event"""
        if ev.Buttons != MB_LEFT:
            return None
        if require_double_click and ev.ClickCount != 2:
            return None
        row = self._ctr.getRowAtPoint(ev.X, ev.Y)
        if row == -1:
            return None

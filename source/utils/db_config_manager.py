...
class DatabaseConfigManager(ConfigManager):
    ...
    def get_connection_params(self):
        ...
        try:
            ...
            if not self.config_file_exists():
                logger.warning("No database configuration file found")
                return None
    ...
    def prompt_configuration(self):
        ...
        try:
            ...
            try:
                MsgBox("Database configuration dialog is unavailable. Please check your LibrePy installation and restart the application.", "Critical Error")
            except:
                pass
            return False
        except Exception as e:
            import traceback
            logger.error(f"Critical: Unexpected error importing DBDialog - {str(e)}")
            logger.debug(traceback.format_exc())
            
            try:
                MsgBox("Database configuration system failed to initialize. Please restart the application.", "Critical Error")
            except:
                pass
            return False
        ...
        try:
            ...
            try:
                ...
                logger.error("getDefaultContext() not available - LibrePy environment issue")
                try:
                    MsgBox("LibrePy environment error. Please restart LibreOffice.", "Environment Error")
                except:
                    pass
                return False
                
            dlg = DBDialog(ctx, None, logger)
            ...
            try:
                MsgBox("An error occurred while showing the database configuration dialog. Please check the logs for details.", "Configuration Error")
            except:
                pass
            return False 